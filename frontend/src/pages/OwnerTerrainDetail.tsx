...
const OwnerTerrainDetail = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const acceptedReservations = reservationsData.filter((r: any) => r.statut === 'acceptee').length;
    ...
    return (
      <div className="page-container flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => navigate("/proprietaire")}>Retour aux terrains</Button>
        </div>
      </div>
    );
  }
  ...
  const terrainImage = terrain.photos?.[0] || defaultImages[(terrain.id - 1) % defaultImages.length];
                  ...
                  <div key={r.id} className="stat-card flex items-center gap-3">
                    ...
                    <div className="text-right">
                      ...
                      <Badge className={`text-[10px] mt-1 ${
                        ...
                        {r.statut === 'acceptee' ? 'Acceptée' : r.statut === 'en_attente' ? 'En attente' : 'Annulée'}
                      </Badge>
                    </div>
                  </div>
                ))}
