...
import { LayoutDashboard, BarChart3, Map, ShieldCheck } from "lucide-react";
...
const TABS = [
  ...
  { to: "/backoffice/proprietaire/sante", label: "Sante", icon: ShieldCheck, match: "sante" },
...
function tabActive(match: string, pathname: string, search: string) {
  ...
  if (match === "sante") return pathname.startsWith("/backoffice/proprietaire/sante");
  return false;
}
  ...
  const Sidebar = (
    <aside className="flex flex-col w-[220px] min-h-full bg-[var(--color-sidebar)] text-white shrink-0">
      <div className="px-5 py-6 border-b border-white/10">
        ...
        <p className="text-[11px] text-white/45 mt-1">Espace proprietaire</p>
      </div>
