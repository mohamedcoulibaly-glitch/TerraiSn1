import { ArrowLeft, User, Mail, Phone, MapPin, LogOut, ChevronRight, Bell, HelpCircle, Shield, Edit, Calendar } from "lucide-react";
...
const menuItems = [
  { icon: Calendar, label: "Mes réservations", path: "/reservations", desc: "Historique et réservations à venir" },
  { icon: Bell, label: "Notifications", path: "/profil/notifications", desc: "Gérer vos alertes" },
  { icon: Shield, label: "Sécurité", path: "/profil/securite", desc: "Mot de passe et confidentialité" },
  { icon: HelpCircle, label: "Aide et support", path: "/profil/aide", desc: "FAQ et contact" },
];
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className="page-container flex flex-col items-center justify-center gap-4">
        ...
        <Button variant="hero" onClick={() => navigate("/login")}>Se connecter</Button>
      </div>
    );
  }
  ...
  const roleLabel =
    user.role === "super_admin" || user.role === "superadmin"
      ...
      : user.role === "proprietaire"
        ? "Propriétaire"
        : user.role === "gerant" || user.role === "employe"
          ? "Gérant"
          : "Joueur";
