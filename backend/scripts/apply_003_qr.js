...
(async () => {
  ...
  if (!hasColumn(db, 'reservations', 'qr_code_url')) {
    runSql(db, 'ALTER TABLE reservations ADD COLUMN qr_code_url TEXT');
    console.log('Applied: qr_code_url');
  } else {
    console.log('Skipped: qr_code_url already present');
  }
  saveDb();
  const cols = queryAll(db, 'PRAGMA table_info(reservations)')
